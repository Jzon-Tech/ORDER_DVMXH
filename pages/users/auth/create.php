<?php 
	...
	if(isset($_POST['jzon']) && !isset($_SESSION['username'])){
		// $res['error'] = "Tính năng đăng kí đã bị vô hiệu hóa";
		// die(json_encode($res));
		...
		if(!$repassword){
			$res['error'] = "Vui lòng nhập lại mật khẩu";
			die(json_encode($res));
		}

		$arr_restricted = ['admin'];

		if(in_array($username, $arr_restricted)){
			$res['error'] = "Tên đăng nhập chứa từ khóa cấm";
			die(json_encode($res));
		}
		...
		if(!$info){
			if(strlen($password) > 6){
				if($repassword == $password){
					if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `users` WHERE `ip` = '".myIP()."' ")) <= 3){
						$create_acc = mysqli_query($conn, "INSERT INTO `users` (`username`, `password`, `created_time`, `ip`, `day`, `month`, `year`) VALUES ('$username', '".typePassword($password)."', '".time()."', '".myIP()."', '".date('d')."', '".date('m')."', '".date('Y')."') ");
						if($create_acc){
							
							if(setting('nofication_new_member') == 'yes'){
								sendTele(genContent("$username vừa tham gia thành công"));
							}
							
							$res['success'] = "Tạo tài khoản thành công, vui lòng đăng nhập";
							die(json_encode($res));
						}else{
							$res['error'] = ERROR_MSG;
							die(json_encode($res));
				...
				$res['error'] = "Mật khẩu không hợp lệ";
				die(json_encode($res));
			}
		}else{
			$res['error'] = "Tên đăng nhập không khả dụng";
			die(json_encode($res));
		}
	}
                            ...
                            <div class="col-xl-12">
                                <div class="auth-form">
                                    ...
                                    <div class="text-center">
                                        <button type="submit" class="btn btn-primary btn-block" data-loading='<i class="fas fa-spinner fa-pulse"></i> ĐANG TẠO TÀI KHOẢN' onclick="moduleCreateAccount(this)"><i class="fas fa-plus"></i> TẠO TÀI KHOẢN</button>
                                    </div>
                                    <div class="new-account mt-3">
                                        <p>Đã có tài khoản? <a class="text-primary" href="/auth/login">Đăng nhập ngay</a></p>
                                    </div>
                                </div>
                            </div>
                        </div>
